...
class class_News {
    ...
    function view() {
        $this->tpl->define_dynamic('index', $_SESSION['INTERFACE'] . '/news/view.tpl');
        $this->tpl->define_dynamic('empty', $_SESSION['INTERFACE'] . '/news/empty.tpl');
        $this->tpl->define_dynamic('menu', 'index');
        $sql_sitemap = $this->db->queryAllRecords("SELECT * FROM news WHERE id<>'1'");
        $_numrow1 = 0;        
                           ...
                           <table width="100%" cellspacing="0" bgcolor="#dedede" border="0">
                                <tr>
                                    ...
                                    <td width="41" align="center" valign="middle">
                                        <a href="/news.php?id=add" target="settings"><img src="/images/add.gif" width="10" height="10" border="0"></a>
                                        <a href="/news.php?id=edit&ssid='.$map['id'].'" target="main"><img src="/images/folder_2.gif" width="10" height="10" border="0"></a>
                                        <a href="/news.php?id=delete&ssid='.$map['id'].'" onclick="if (confirm(\'¬ы действительно хотите удалить новость '.$map['title'].'?\')) {return true} else {return false};"><img src="/images/delete.gif" width="10" height="10" border="0"></a>
                                    </td>
                                </tr>
                           </table>
                       </td>
    ...
    function add() {
        $this->tpl->define_dynamic('add_menu', $_SESSION['INTERFACE'] . '/news/add.tpl');
        ...
        if(isset($_POST['save'])) {
            ...
            $this->db->query("INSERT INTO news SET 
                                                        `title`='".$_POST['_title']."',
                                                        `public_date`='".$dd."',
            ...
            header("Location: /news.php?id=view");
        }
        return true;
    }

    function edit() {
        $this->tpl->define_dynamic('edit_menu', $_SESSION['INTERFACE'] . '/news/edit.tpl');
        if(isset($_POST['save'])) {
            $_date = explode('.', $_POST['_date']);
            $dd = $_date[2] . '-' . $_date[1] . '-' . $_date[0];
            ...
            $sql = $this->db->queryOneRecord("SELECT * FROM news WHERE id='".$_GET['ssid']."'");
    ...
    function delete() {
        $this->db->query("DELETE FROM news WHERE id='".$_GET['ssid']."'");
        header("Location: /news.php?id=view");
    }
}
?>
