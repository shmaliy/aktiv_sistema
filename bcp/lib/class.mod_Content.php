...
class mod_Content{
    function __construct($db,$tpl) {
        ...
        foreach($SQL AS $start) {
         if(!isset($_GET['id']) && $start['href'] == 'main'){header("Location:/");}
        }
    }

    function view() {
        if(!isset($_GET['id']) || @$_GET['id'] == 'main') {$this->tpl->define_dynamic('index', 'main.tpl');}
		if(isset($_GET['id']) && @$_GET['id'] !== 'main' && @$_GET['id'] == 'company') {$this->tpl->define_dynamic('index', 'des_company.tpl');}
		if(isset($_GET['id']) && @$_GET['id'] !== 'main' && @$_GET['id'] == 'services') {$this->tpl->define_dynamic('index', 'des_services.tpl');}
		if(isset($_GET['id']) && @$_GET['id'] !== 'main' && @$_GET['id'] == 'tools') {$this->tpl->define_dynamic('index', 'des_tools.tpl');}
		if(isset($_GET['id']) && @$_GET['id'] !== 'main' && @$_GET['id'] == 'sitemap') {$this->tpl->define_dynamic('index', 'sitemap.tpl');}
		if(isset($_GET['id']) && @$_GET['id'] !== 'main' && @$_GET['id'] == 'news' && !isset($_GET['ssid'])) {$this->tpl->define_dynamic('index', 'news.tpl');}
		if(isset($_GET['id']) && @$_GET['id'] !== 'main' && @$_GET['id'] == 'news' && isset($_GET['ssid'])) {$this->tpl->define_dynamic('index', 'news_detail.tpl');}
		if(isset($_GET['id']) && @$_GET['id'] !== 'main' && @$_GET['id'] == 'actions' && !isset($_GET['ssid'])) {$this->tpl->define_dynamic('index', 'actions.tpl');}
		if(isset($_GET['id']) && @$_GET['id'] !== 'main' && @$_GET['id'] == 'actions' && isset($_GET['ssid'])) {$this->tpl->define_dynamic('index', 'actions_detail.tpl');}
		if(isset($_GET['id']) && @$_GET['id'] !== 'main' && @$_GET['id'] == 'base' && !isset($_GET['ssid'])) {$this->tpl->define_dynamic('index', 'base.tpl');}
		if(isset($_GET['id']) && @$_GET['id'] !== 'main' && @$_GET['id'] == 'base' && isset($_GET['ssid'])) {$this->tpl->define_dynamic('index', 'base_detail.tpl');}
        ...
        if(!$this->db->numRows(@$sql_content)) {
            @$sql_default = $this->db->queryOneRecord("SELECT * FROM meta_default");
                ...
                if(@$_GET['id'] == 'news') {$def_txt = ' - Новости';}
                if(@$_GET['id'] == 'feedback') {$def_txt = ' - Форма обратной связи';}
                if(@$_GET['id'] == 'catalog') {$def_txt = ' - Каталог продукции';}
		    ...
		    foreach($sql_content AS $str_content) {
		        $content = eregi_replace('font-size: smaller', 'font-size: 8pt',$str_content['body']);
		        $content = eregi_replace('font-size: larger', 'font-size: 14pt',$content);
		        $content = eregi_replace('font-size: xx-small', 'font-size: 9pt',$content);
		        $content = eregi_replace('font-size: x-small', 'font-size: 10pt',$content);
		        $content = eregi_replace('font-size: small', 'font-size: 12pt',$content);
		        $content = eregi_replace('font-size: medium', 'font-size: 17pt',$content);
		        $content = eregi_replace('font-size: large', 'font-size: 18pt',$content);
		        $content = eregi_replace('font-size: x-large', 'font-size: 24pt',$content);
		        $content = eregi_replace('font-size: xx-large', 'font-size: 30pt',$content);
		        $content = eregi_replace('"images/', '"/images/',$content);
		        $content = eregi_replace("'images/", '"/images/',$content);
